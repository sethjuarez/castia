...
test("request handler serves committed playground client asset", async () => {
    const handleRequest = createRequestHandler({
        extensionRoot: fileURLToPath(new URL("..", import.meta.url)),
    });
    const response = jsonResponse();

    await handleRequest({ method: "GET", url: "/assets/playground-client.js" }, response, {});

    assert.equal(response.status, 200);
    assert.equal(response.headers["Content-Type"], "application/javascript; charset=utf-8");
    assert.match(response.body, /Generated by `npm run build`/);
    assert.match(response.body, /function renderMarkdown/);
});

test("request handler caches static assets after first read", async () => {
    const root = await mkdtemp(join(tmpdir(), "playground-route-assets-"));
    const dist = join(root, "renderer", "dist");
    await mkdir(dist, { recursive: true });
    await writeFile(join(dist, "playground-client.js"), "first asset", "utf8");
    try {
        const handleRequest = createRequestHandler({ extensionRoot: root });
        const first = jsonResponse();
        const second = jsonResponse();

        await handleRequest({ method: "GET", url: "/assets/playground-client.js" }, first, {});
        await writeFile(join(dist, "playground-client.js"), "second asset", "utf8");
        await handleRequest({ method: "GET", url: "/assets/playground-client.js" }, second, {});

        assert.equal(first.status, 200);
        assert.equal(second.status, 200);
        assert.equal(first.body, "first asset");
        assert.equal(second.body, "first asset");
    } finally {
