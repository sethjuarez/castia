...
// Do not edit this file directly.
    ...
    const isStartupReadinessPending = function isStartupReadinessPending(state) {
    return Boolean(
    ...
    const localReadinessState = function localReadinessState(state) {
    ...
    const localStartupText = function localStartupText(localRun) {
    ...
    const localFailureDetail = function localFailureDetail(localRun) {
    const failure = localRun?.failure;
    if (!failure) return "";
    return [
        failure.rootCause,
    ...
    const responseDetailsKey = function responseDetailsKey(turn, index = 0) {
    ...
    const responseDetailsPanelId = function responseDetailsPanelId(detailsKey) {
    ...
    const composerGate = function composerGate(state, { activeView = "chat", inFlight = false } = {}) {
    if (activeView !== "chat") {
    ...
    const shouldOpenProjectEndpointDialog = function shouldOpenProjectEndpointDialog(state) {
    return Boolean(state?.projectEndpointPrompt?.open);
};
    const responseText = function responseText(body) {
    ...
    const responseDisplayState = function responseDisplayState(response) {
    ...
    const copyableAnswerText = function copyableAnswerText(response) {
    ...
    const parseResponseEnvelopeString = function parseResponseEnvelopeString(value) {
    ...
    const isResponseEnvelope = function isResponseEnvelope(body, seen = new Set()) {
    ...
    const textOrEmpty = function textOrEmpty(value) {
    return typeof value === "string" && value.trim() ? value : "";
};
    const textFromResponseBody = function textFromResponseBody(body, seen = new Set()) {
    ...
    const textFromResponsesOutput = function textFromResponsesOutput(output) {
