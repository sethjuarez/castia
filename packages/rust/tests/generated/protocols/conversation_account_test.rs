...
{}
"####;
    let instance: ConversationAccount = serde_json::from_str(json)
    ...
    assert_eq!(instance, ConversationAccount::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: ConversationAccount = serde_json::from_value(value)
...
fn test_conversation_account_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(ConversationAccount::from_json("{", &ctx).is_err(), "malformed JSON must be rejected instead of silently defaulting");
}
