...
use castia::model::ChannelAccount;
use castia::model::context::{LoadContext, SaveContext};

#[test]
fn test_channel_account_load_json() {
    let json = r####"
{}
"####;
    let ctx = LoadContext::default();
    let result = ChannelAccount::from_json(json, &ctx);
...
#[test]
fn test_channel_account_load_yaml() {
    let yaml = r####"
{}

"####;
    let ctx = LoadContext::default();
    let result = ChannelAccount::from_yaml(yaml, &ctx);
...
#[test]
fn test_channel_account_roundtrip() {
    let json = r####"
{}
"####;
    let load_ctx = LoadContext::default();
    let result = ChannelAccount::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_channel_account_serde_roundtrip() {
    let json = r####"
{}
"####;
    let instance: ChannelAccount = serde_json::from_str(json)
    ...
    assert_eq!(instance, ChannelAccount::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: ChannelAccount = serde_json::from_value(value)
        .expect("serde should re-deserialize");
    assert_eq!(instance, reparsed, "serde round-trip must be stable");
}

#[test]
fn test_channel_account_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(ChannelAccount::from_json("{", &ctx).is_err(), "malformed JSON must be rejected instead of silently defaulting");
}
