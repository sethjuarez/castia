...
use castia::model::Mention;
use castia::model::context::{LoadContext, SaveContext};

#[test]
fn test_mention_load_json() {
    let json = r####"
{}
"####;
    let ctx = LoadContext::default();
    let result = Mention::from_json(json, &ctx);
...
#[test]
fn test_mention_load_yaml() {
    let yaml = r####"
{}

"####;
    let ctx = LoadContext::default();
    let result = Mention::from_yaml(yaml, &ctx);
...
"####;
    ...
    let result = Mention::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_mention_serde_roundtrip() {
    let json = r####"
{}
"####;
    let instance: Mention = serde_json::from_str(json)
    ...
    assert_eq!(instance, Mention::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: Mention = serde_json::from_value(value)
        .expect("serde should re-deserialize");
    assert_eq!(instance, reparsed, "serde round-trip must be stable");
}

#[test]
fn test_mention_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(Mention::from_json("{", &ctx).is_err(), "malformed JSON must be rejected instead of silently defaulting");
}
