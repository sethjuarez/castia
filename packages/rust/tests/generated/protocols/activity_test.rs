...
use castia::model::Activity;
use castia::model::context::{LoadContext, SaveContext};

#[test]
fn test_activity_load_json() {
    let json = r####"
{}
"####;
    let ctx = LoadContext::default();
    let result = Activity::from_json(json, &ctx);
...
#[test]
fn test_activity_load_yaml() {
    let yaml = r####"
{}

"####;
    let ctx = LoadContext::default();
    let result = Activity::from_yaml(yaml, &ctx);
...
"####;
    ...
    let result = Activity::from_json(json, &load_ctx);
...
{}
"####;
    let instance: Activity = serde_json::from_str(json)
    ...
    assert_eq!(instance, Activity::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: Activity = serde_json::from_value(value)
        .expect("serde should re-deserialize");
    assert_eq!(instance, reparsed, "serde round-trip must be stable");
}

#[test]
fn test_activity_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(Activity::from_json("{", &ctx).is_err(), "malformed JSON must be rejected instead of silently defaulting");
}
