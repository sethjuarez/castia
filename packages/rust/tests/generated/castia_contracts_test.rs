...
"####;
    let instance: CastiaContracts = serde_json::from_str(json)
    ...
    assert_eq!(instance, CastiaContracts::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: CastiaContracts = serde_json::from_value(value)
...
fn test_castia_contracts_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(CastiaContracts::from_json("{", &ctx).is_err(), "malformed JSON must be rejected instead of silently defaulting");
}
