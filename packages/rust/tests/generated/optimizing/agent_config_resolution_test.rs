...
use castia::model::AgentConfigResolution;
use castia::model::context::{LoadContext, SaveContext};

#[test]
fn test_agent_config_resolution_load_json() {
    let json = r####"
{
  "source": "sample"
}
"####;
    ...
    let result = AgentConfigResolution::from_json(json, &ctx);
    assert!(result.is_ok(), "Failed to load from JSON: {:?}", result.err());
    let instance = result.unwrap();
    assert_eq!(instance.source, "sample");
}

#[test]
fn test_agent_config_resolution_load_yaml() {
    let yaml = r####"
source: sample

"####;
    let ctx = LoadContext::default();
    let result = AgentConfigResolution::from_yaml(yaml, &ctx);
...
#[test]
fn test_agent_config_resolution_roundtrip() {
    let json = r####"
{
  "source": "sample"
}
"####;
    ...
    let result = AgentConfigResolution::from_json(json, &load_ctx);
...
#[cfg(feature = "serde")]
#[test]
fn test_agent_config_resolution_serde_roundtrip() {
    let json = r####"
{
  "source": "sample"
}
"####;
    let instance: AgentConfigResolution = serde_json::from_str(json)
    ...
    assert_eq!(instance, AgentConfigResolution::load_from_value(&canonical, &LoadContext::default()), "serde deserialize must equal canonical load_from_value");
    ...
    let reparsed: AgentConfigResolution = serde_json::from_value(value)
        .expect("serde should re-deserialize");
    assert_eq!(instance, reparsed, "serde round-trip must be stable");
}

#[test]
fn test_agent_config_resolution_from_json_invalid() {
    let ctx = LoadContext::default();
    assert!(AgentConfigResolution::from_json("{", &ctx).is_err(), "malformed JSON must be rejected instead of silently defaulting");
}
