...
When `azd ai agent invoke` succeeds but does not print the expected answer text,
use the trace ID printed by azd as the App Insights `operation_Id`. This tells
you whether Castia, the model call, and the platform wrapper completed before
you blame the application.

Start with hosted session logs from the agent root:

```powershell
$env:AZURE_DEV_USER_AGENT = "microsoft_foundry_skill"
azd ai agent sessions list <service-name> --output table
azd ai agent monitor <service-name> --tail 120
```

Then query the App Insights resource connected to the project. If the azd
environment does not expose it, list likely resources by resource group:
...
Use one-line KQL strings with `az monitor app-insights query`; PowerShell and
the Azure CLI can otherwise strip quotes from multiline `let` statements.

```powershell
$operationId = "<trace-id-from-azd-invoke>"
$appInsightsId = "<application-insights-resource-id>"
$q = "requests | where timestamp > ago(1h) | where operation_Id == '$operationId' | project timestamp, name, resultCode, success, duration, operation_Id, customDimensions | order by timestamp asc"
az monitor app-insights query --ids $appInsightsId --analytics-query $q -o json

$q = "dependencies | where timestamp > ago(1h) | where operation_Id == '$operationId' | project timestamp, name, type, target, resultCode, success, duration, operation_ParentId, customDimensions | order by timestamp asc"
az monitor app-insights query --ids $appInsightsId --analytics-query $q -o json

$q = "traces | where timestamp > ago(1h) | where operation_Id == '$operationId' | project timestamp, severityLevel, message, operation_ParentId, customDimensions | order by timestamp asc"
az monitor app-insights query --ids $appInsightsId --analytics-query $q -o json
```

Interpret the rows like this:

- `requests` row `POST /responses` with `resultCode == 200` means the Castia
  HTTP adapter sent a successful Responses payload.
- `requests` or `dependencies` row `invoke_agent` with `success == True` means
  the Foundry hosted-agent wrapper completed. Its `customDimensions` usually
  include `azure.ai.agentserver.response_id`, conversation ID, session ID, and
  whether the wrapper used streaming.
- `dependencies` row `chat <model>` with `success == True`,
  `gen_ai.output.messages`, `finish_reason == "completed"`, and token counts
  means the model call completed and returned an assistant message.
- App Insights normally records GenAI content shape and IDs, not raw answer
  text. If all rows above succeeded but `azd ai agent invoke` printed no answer,
  the likely issue is CLI/platform response rendering, not the Castia handler.
