...
class AgentConfig:
    ...
    def apply_tools(self, tools: list[Tool]) -> list[Tool]:
...
def apply_optimized_tools(
    tools: list[Tool], tool_definitions: tuple[dict, ...] | list[dict]
) -> list[Tool]:
...
def tools_json(tools: list[Tool]) -> list[dict]:
    ...
    the agent's registered tools (see ``python -m castia optimize``). This is
    the inverse of :func:`apply_optimized_tools`.
    """
    return [
        {
            "type": "function",
...
def configured_model(
    ...
    optimizer-injected candidate) ``model`` + ``instructions`` into every
    :class:`~castia.model.Model` it builds, so an optimized prompt takes
    effect without any handler change::
    ...
    def provider() -> Model:
        ...
        return Model(resolved.model, instructions=resolved.instructions)
