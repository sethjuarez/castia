"""Unit tests for instruction threading through ``castia.model.Model``.

The optimizer tunes an agent's system prompt, but that only matters if the
resolved ``instructions`` actually reach the Responses API call. These cases
pin that ``respond`` / ``stream`` / ``respond_with_tools`` all pass
``instructions=`` into ``responses.create(...)``. ``Model`` is built via
``__new__`` with a fake OpenAI client so no Azure client or network is touched.
"""
...
def _model(instructions: str | None) -> tuple[Model, dict]:
    sink: dict = {}
